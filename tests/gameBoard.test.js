...
test.skip('did all their ships sunk?', ()=> { // was tested with just one boat
  const playerBoard=gameBoards();
  playerBoard.receiveAttack( 4, 4);
...
test('is gameboard too heavy?', ()=> {
  const playerBoard=gameBoards();
  expect(playerBoard).toMatch('anything'); // gameBoard was taking infinite
});
